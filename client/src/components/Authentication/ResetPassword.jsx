...
function ResetPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (password !== confirmPassword) {
            return setOpenSnackbar({ open: true, type: 'error', message: 'Passwords do not match.' });
        }

        try {
            await axios.post(`${process.env.REACT_APP_SERVER_URL}/api/auth/reset-password/${token}`, { password });
            setOpenSnackbar({ open: true, type: 'success', message: 'Password reset successfully. Redirecting...' });
        ...
        <div className="relative bg-gradient-to-br from-gray-900 to-blue-900 min-h-screen w-full flex flex-col overflow-hidden">
            ...
            <div className="flex flex-1 items-center justify-center w-full">
                <div className="flex flex-col w-full max-w-lg bg-[#232b3b]/90 rounded-2xl border border-gray-700 shadow-2xl p-10 gap-6 items-center justify-center">
                    <span className="text-blue-100 text-3xl font-bold antialiased text-center mb-2">Reset Password</span>
                    <span className="text-blue-200 text-md font-extralight antialiased text-center mb-4">Enter your new password below to reset your account password.</span>
